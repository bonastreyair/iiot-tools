...
#define FREERTOS_SYSTEM_TASKS 7 // min tasks needed to FreeRTOS system to work
...
void setup() {
  ...
  // Create a task that will execute tasksManager() function, with no
  // parameters, with default priority on any available core
  xTaskCreate(tasksManager,     /* task function */
...
void loop() {
  LOG(String(uxTaskGetNumberOfTasks() - FREERTOS_SYSTEM_TASKS) +
      " running tasks");
  delay(1000);
}
...
void tasksManager(void *pvParameters) {
  ...
  // Create a task that will execute test1Task() function, with no parameters,
  // with default priority on any available core
  xTaskCreate(test1Task,        /* task function */
              "task1",          /* name of task */
              TASK_STACK_SIZE,  /* stack size of task */
              NULL,             /* no parameter for the task */
              DEFAULT_PRIORITY, /* sets the priority of the task */
  ...
  // Create a task that will execute test2Task() function, with parameters, with
  // default priority and executed on core 0
  ...
  vTaskDelay(
      6000 /
      portTICK_PERIOD_MS); // Blocks the task and lets other tasks to work
  vTaskDelete(xTask1); // task1 is the defined handler kills the task and frees
                       // up memory space
...
void test1Task(void *pvParameters) {
  ...
  ulTaskNotifyTake(
      pdTRUE,
      portMAX_DELAY); // Blocks indefinitely until it receives a notification
  LOG("running on core " + String(xPortGetCoreID()));
...
void test2Task(void *pvParameters) {
  ...
  LOG("parameters: " + String(params.delayTime) + ", " +
      String(params.id_example) + ", " + String(params.floatNum_example));
  for (;;) {
